...
class Elbishion_Export {
	...
	public static function maybe_export() {
		...
		$args = array(
			...
			'user_scope' => isset( $_GET['user_scope'] ) ? sanitize_key( wp_unslash( $_GET['user_scope'] ) ) : '',
			'user_id'   => isset( $_GET['user_id'] ) ? absint( $_GET['user_id'] ) : '',
			'page_url'  => isset( $_GET['page_url'] ) ? sanitize_text_field( wp_unslash( $_GET['page_url'] ) ) : '',
	...
	private static function send_csv( $args ) {
		...
		foreach ( $rows as $row ) {
			fputcsv(
				...
				array(
					$row->id,
					isset( $row->source_plugin ) && $row->source_plugin ? $row->source_plugin : ( $row->source ?? 'api' ),
					$row->form_name,
					$row->submitted_data,
					$row->page_url,
					$row->user_ip,
